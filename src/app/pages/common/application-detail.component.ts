  ...
  template: `
    <div class="dialog-header">
      <h2 mat-dialog-title>
        ...
        <app-chip [status]="data.application.status!" />
      </h2>
...
export class ApplicationDetailComponent implements OnInit {
  protected readonly data =
    inject<ApplicationDetailData>(MAT_DIALOG_DATA);
  private readonly dialogRef = inject(
    MatDialogRef<ApplicationDetailComponent>
  );
  ...
  private loadEvaluations() {
    this.applicationService
      .getApplicationReviews(this.data.application.id!)
      .subscribe((page) => {
        this.evaluations.set(page.content);
        if (page.content.length > 0) {
          const avg =
            page.content.reduce((sum, e) => sum + (e.score ?? 0), 0) /
            page.content.length;
  ...
  submitEvaluation() {
    ...
    this.applicationService
      ...
      .subscribe({
        ...
        error: () => {
          this.snackBar.open('Failed to submit evaluation', 'Close', {
            duration: 3000,
          });
        },
      });
