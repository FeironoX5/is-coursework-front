...
import { ApplicationDetailComponent, type ApplicationDetailData } from '../../components/application-detail/application-detail.component';

import { ResidenceProgramService } from '../../api/residence-program.service';
import { ApplicationService } from '../../api/application.service';
import type { ProgramDto, ApplicationDto, ApplicationEvaluationDto } from '../../api/models';
...
@Component({
  ...
  imports: [
    ...
    MatCheckboxModule, MatCardModule, MatChipsModule, MatDialogModule,
  ...
  template: `
    <div class="page-container">
      @if (loading()) {
        ...
        <app-page-header
          [title]="'Applications Ranking: ' + program()!.previewDto?.title"
          subtitle="Review evaluations and select artists"
        >
          <button mat-button (click)="router.navigate(['/residences/me/programs', programId()])">
          ...
          <mat-card-content>
            <div class="quota-row">
              <div class="quota-item">
                ...
                <span class="quota-value">{{ program()!.peopleQuota ?? '—' }}</span>
              </div>
              <div class="quota-item">
                <span class="quota-label">Budget</span>
                <span class="quota-value">€{{ program()!.budgetQuota?.toLocaleString() ?? '—' }}</span>
              </div>
              <div class="quota-item">
                <span class="quota-label">Selected</span>
                <span class="quota-value highlight">{{ selection.selected.length }}</span>
              </div>
            </div>
          </mat-card-content>
        </mat-card>
        ...
        @if (rankedApps().length === 0) {
          ...
          <table mat-table [dataSource]="rankedApps()" class="ranking-table">
            ...
            <ng-container matColumnDef="rank">
              ...
              <td mat-cell *matCellDef="let row; let i = index" class="rank-cell">
                #{{ i + 1 }}
              </td>
            </ng-container>
            ...
            <ng-container matColumnDef="id">
              ...
              <td mat-cell *matCellDef="let row">{{ row.application.id }}</td>
            </ng-container>
            ...
            <ng-container matColumnDef="score">
              ...
              <td mat-cell *matCellDef="let row" class="score-cell">
                <span class="score-badge">{{ row.averageScore }}</span>
              </td>
            </ng-container>
            ...
            <ng-container matColumnDef="status">
              ...
              <td mat-cell *matCellDef="let row">
                <app-status-badge [status]="row.application.status!" />
              </td>
            </ng-container>
            ...
            <ng-container matColumnDef="actions">
              ...
              <td mat-cell *matCellDef="let row">
                <button mat-icon-button (click)="viewDetails(row)" aria-label="View details">
            ...
            <tr mat-row *matRowDef="let row; columns: columns;" class="ranking-row"></tr>
          </table>
            ...
            <div class="selection-actions">
              <mat-card appearance="outlined">
                <mat-card-content class="actions-content">
                  <span class="selection-text">{{ selection.selected.length }} selected</span>
                  <div class="action-buttons">
                    <button mat-flat-button color="primary" (click)="approveSelected()">
                      <mat-icon>check</mat-icon> Approve Selected
                    </button>
                    <button mat-stroked-button (click)="reserveSelected()">
  ...
  styles: [`
    ...
    .quota-label { display: block; font-size: 12px; color: rgba(0,0,0,.54); margin-bottom: 4px; }
    .quota-value { display: block; font-size: 24px; font-weight: 600; color: rgba(0,0,0,.87); }
    .quota-value.highlight { color: #7c4dff; }
...
export class ApplicationsRankingPage implements OnInit {
  ...
  readonly columns = ['select', 'rank', 'id', 'score', 'evaluations', 'status', 'actions'];
  readonly selection = new SelectionModel<RankedApplication>(true, []);
  ...
  private loadRanking(programId: number) {
    ...
    this.applicationService.getEvaluatedApplications(programId).subscribe(async page => {
      // For each application, fetch evaluations and calculate average
      const ranked: RankedApplication[] = [];

      for (const app of page.content) {
        const evals = await this.applicationService.getApplicationReviews(app.id!).toPromise();
  ...
  isAllSelected(): boolean {
    return this.selection.selected.length === this.rankedApps().length;
  }
  ...
  approveSelected() {
    ...
    if (quota && this.selection.selected.length > quota) {
      if (!confirm(`You selected ${this.selection.selected.length} artists but quota is ${quota}. Continue?`)) {
        return;
      }
    }
  ...
  rejectSelected() {
    if (!confirm(`Reject ${this.selection.selected.length} applications?`)) return;
  ...
  private processSelection(action: 'approve' | 'reserve' | 'reject') {
    ...
    const apiCall = action === 'approve'
      ? (id: number) => this.applicationService.approveApplication(id)
      : action === 'reserve'
      ? (id: number) => this.applicationService.reserveApplication(id)
      : (id: number) => this.applicationService.rejectApplication(id);
      ...
      apiCall(id).subscribe({
        next: () => {
          ...
          if (completed === ids.length) {
            this.snackBar.open(`${ids.length} applications ${action}d`, 'Close', { duration: 2000 });
            this.selection.clear();
